...
HFBackend    — local open model via transformers (default openai/gpt-oss-20b),
               for measuring a non-hardened, injection-vulnerable target on GPU
               hardware. Enabled with MODEL_BACKEND=hf. See docs/HF_SETUP.md.
OpenAIBackend— real black-box model via the OpenAI API (MODEL_BACKEND=openai).
"""
...
class HFBackend:
    """Local open-model backend via transformers.

    Targets the paper's non-hardened, injection-vulnerable models (default
    openai/gpt-oss-20b, which fits a 16 GB VRAM box in its native MXFP4
    quantization). Uses the SAME message framing as the OpenAI backend
    (build_chat_messages), so ASR is comparable across backends, then applies
    the model's own chat template.

    For a low-resource machine (e.g. this Mac), set HF_MODEL to a small model
    such as Qwen/Qwen2.5-1.5B-Instruct.
    """
    ...
    def __init__(self) -> None:
        ...
        self.model_id = os.environ.get("HF_MODEL", "openai/gpt-oss-20b")
        self.flatten = os.environ.get("HF_FLATTEN", "1") != "0"
        self.max_new_tokens = int(os.environ.get("HF_MAX_NEW_TOKENS", "300"))
    ...
    def generate(self, transcript: Transcript, defense_on: bool) -> ModelOutput:
        messages = build_chat_messages(transcript, defense_on, self.flatten)
        ...
        notes = [
            f"model={self.model_id} flatten={self.flatten} "
            f"defense={'on' if defense_on else 'off'}",
            f"secret {'LEAKED' if hijacked else 'withheld'}",
        ]
        return ModelOutput(reply=reply, hijacked=hijacked, notes=notes)


def build_chat_messages(transcript: Transcript, defense_on: bool,
                        flatten: bool = True) -> list[dict]:
    """Map a sealed transcript to chat messages (role/content dicts).

    Shared by every real backend (OpenAI API, local HF chat template) so they
    all present the *same* defense framing — the seal contract, flattening, and
    escaped content — making cross-backend ASR comparisons apples-to-apples.
    Module-level (not a method) so it can be unit-tested without a model.
    """
